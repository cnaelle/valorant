<?php
/**
 * CoCart Pro - Display notices in the WordPress admin.
 *
 * @author   Sébastien Dumont
 * @category Admin
 * @package  CoCart Pro/Admin/Notices
 * @license  GPL-2.0+
 */
...
if ( ! class_exists( 'CoCart_Pro_Admin_Notices' ) ) {

	class CoCart_Pro_Admin_Notices {
		 ...
		 * @access public
		 */
		public function check_cocart_pro_dependency() {
			...
			if ( ! in_array( $screen_id, CoCart_Pro_Admin::cocart_get_admin_screens() ) ) {
				return false;
			}
			...
			else if ( version_compare( COCART_VERSION, CoCart_Pro::$required_cocart, '<' ) ) {
				add_action( 'admin_notices', array( $this, 'required_cocart_version_failed' ) );
				return false;
			}
		} // END check_cocart_pro_dependency()
		...
		public function dont_bug_me() {
			...
			if ( ! empty( $_GET['hide_cocart_pro_review_notice'] ) && current_user_can( 'install_plugins' ) ) {
				add_user_meta( $current_user->ID, 'cocart_pro_hide_review_notice', '1', true );
				$user_hidden_notice = true;
			}

			// Hide the beta notice for two weeks if requested.
			if ( ! empty( $_GET['hide_cocart_pro_beta_notice'] ) && current_user_can( 'install_plugins' ) ) {
				set_transient( 'cocart_pro_beta_notice_hidden', 'hidden', apply_filters( 'cocart_pro_beta_notice_expiration', WEEK_IN_SECONDS * 2 ) );
				$user_hidden_notice = true;
			}
		...
		public function add_notices() {
			...
			$hide_review_notice = get_user_meta( $current_user->ID, 'cocart_pro_hide_review_notice', true );
			...
			// Is this version of CoCart Pro a beta/pre-release?
		...
		public function cocart_not_installed() {
			include_once( COCART_PRO_FILE_PATH . '/includes/admin/views/html-notice-cocart-not-installed.php' );
		...
		public function required_cocart_version_failed() {
			include_once( COCART_PRO_FILE_PATH . '/includes/admin/views/html-notice-required-cocart.php' );
		...
		public function beta_notice() {
			include_once( COCART_PRO_FILE_PATH . '/includes/admin/views/html-notice-trying-beta.php' );
		} // END beta_notice()
		...
		public function plugin_review_notice() {
			...
			include_once( COCART_PRO_FILE_PATH . '/includes/admin/views/html-notice-please-review.php' );
...
return new CoCart_Pro_Admin_Notices();
