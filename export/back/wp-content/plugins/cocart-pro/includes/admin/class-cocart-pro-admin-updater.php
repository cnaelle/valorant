<?php
/**
 * CoCart Pro - Plugin Updater.
 *
 * @author   Sébastien Dumont
 * @category Admin
 * @package  CoCart Pro/Admin/Updater
 * @license  GPL-2.0+
 */
...
if ( ! class_exists( 'CoCart_Pro_Updater' ) ) {
	...
	class CoCart_Pro_Updater {
		...
		public function __construct() {
			$this->config = array(
				'file'               => COCART_PRO_FILE,
				'version'            => COCART_PRO_VERSION,
				'slug'               => COCART_PRO_SLUG,
				'proper_folder_name' => COCART_PRO_SLUG,
			);

			// Hooks into the plugin updater and checks for updates for CoCart Pro.
			add_filter( 'pre_set_site_transient_update_plugins', array( $this, 'api_check' ) );

			// Excludes CoCart Pro from WP.org updates.
			...
			// Add after_plugin_row... action for CoCart Pro.
		...
		public function auto_update_plugin( $should_update, $plugin ) {
			...
			/**
			 * Developers can disable CoCart Pro from auto-updating by filtering the status.
			 * Currently set to "true" at this stage.
			 */
			$should_update = apply_filters( 'cocart_pro_auto_update', true );
		 ...
		 * not auto updates are enabled for CoCart Pro.
		 *
		 * @access public
		...
		public function wp_autoupdates_plugin_column( $column_name, $plugin_file, $plugin_data ) {
			...
			if ( $column_name === 'autoupdates_column' ) {
				...
				// Check the plugin is CoCart Pro only.
				if ( $plugin_file === plugin_basename( $this->config['file'] ) ) {

					// If filtered to disable then notify user auto updates are disabled.
					if ( has_filter( 'cocart_pro_auto_update' ) ) {
						echo '<span class="plugin-autoupdate-disabled"><span class="dashicons dashicons-update" aria-hidden="true"></span> ' . __( 'Automatic updates disabled!', 'cocart-pro' ) . ' </span>';
					} else {
						// TODO: Add licence status checker here.

						/*if ( ! is_cocart_licence_valid() ) {
							echo '<span class="plugin-autoupdate-disabled"><span class="dashicons dashicons-update" aria-hidden="true"></span> ' . __( 'Updates available if licence renewed.', 'cocart-pro' ) . ' </span>';
						} else {*/
							echo '<span class="plugin-autoupdate-enabled"><span class="dashicons dashicons-update" aria-hidden="true"></span> ' . __( 'Automatic updates enabled!', 'cocart-pro' ) . '</span>';
							...
							if ( isset( $plugins_updates->response[ $plugin_file ] ) ) {
								echo sprintf(
									...
									__( 'Update scheduled in %s', 'cocart-pro' ),
									$time_to_next_update
								);
							}
						//}
		...
		/**
		 * Removes CoCart Pro from plugin bulk action for auto updates.
		 *
		 * @access protected
		...
		public function get_latest_release() {
			...
			if ( ! empty( $release ) ) {
				$tagged_version = isset( $release['version'] ) ? $release['version'] : COCART_PRO_VERSION;
			}
		...
		public function get_changelog() {
			...
			return ! empty( $_changelog ) ? $_changelog : __( 'Unable to return the changelog at this time.', 'cocart-pro' );
		} // END get_changelog()
		...
		public function get_faq() {
			return sprintf( __( 'The documentation for CoCart Pro is %1$shere%2$s.', 'cocart-pro' ), '<a href="' . COCART_PRO_DOCUMENTATION_URL . '" target="_blank">', '</a>' );
		} // END get_faq()
		...
		public function allow_prereleases() {
			return apply_filters( 'cocart_pro_allow_prereleases', false );
		} // END allow_prereleases()

		/**
		 * Plugin information callback for CoCart Pro.
		 *
		 * @access  public
		...
		public function get_plugin_info( $response, $action, $args ) {
			...
			if ( $this->allow_prereleases() ) {
				if ( $this->is_stable_version( $new_version ) ) {
					$warning = sprintf( __( '%1$s%3$sThis is the latest stable release%3$s%2$s', 'cocart-pro' ), '<h1>', '</h1>', '<span>&#9888;</span>' );
				}

				if ( $this->is_beta_version( $new_version ) ) {
					$warning = sprintf( __( '%1$s%3$sThis is a beta release%3$s%2$s', 'cocart-pro' ), '<h1>', '</h1>', '<span>&#9888;</span>' );
				}

				if ( $this->is_rc_version( $new_version ) ) {
					$warning = sprintf( __( '%1$s%3$sThis is a pre-release%3$s%2$s', 'cocart-pro' ), '<h1>', '</h1>', '<span>&#9888;</span>' );
				}
			}
			...
			if ( version_compare( WC_VERSION, $this->config['wc_tested_up_to'], '<' ) ) {
				$warning .= '<p><strong>' . sprintf( __( '%1$sThe version of WooCommerce you have installed has not yet been tested with this release.%1$s', 'cocart-pro' ), '<span>&#9888;</span>' ) . '</strong></p>';
			}
		...
		public function upgrader_source_selection( $source, $remote_source, $upgrader ) {
			...
			if ( strstr( $source, '/'. COCART_PRO_SLUG . '-' . COCART_PRO_SLUG . '-' ) ) {
				$corrected_source = trailingslashit( $remote_source ) . trailingslashit( $this->config[ 'proper_folder_name' ] );

				if ( $wp_filesystem->move( $source, $corrected_source, true ) ) {
					return $corrected_source;
				} else {
					return new WP_Error( 'upgrader_source_selection', __( 'Unable to update!', 'cocart-pro' ), 500 );
				}
			}
		...
		/**
		 * Displays update information for CoCart Pro.
		 *
		 * @access public
		...
		public function plugin_row( $file, $plugin_data ) {
			...
			if ( is_network_admin() || ! is_multisite() ) {
				...
				echo '<tr class="plugin-update-tr' . $active_class . ' cocart-pro-custom" id="' . esc_attr( $response->slug . '-update' ) . '" data-slug="' . esc_attr( $response->slug ) . '" data-plugin="' . esc_attr( $file ) . '"><td colspan="3" class="plugin-update colspanchange"><div class="update-message notice inline ' . $notice_type . ' notice-alt"><p>';

				if ( ! current_user_can( 'update_plugins' ) ) {
					/* translators: 1: plugin name, 2: details URL, 3: additional link attributes, 4: version number */
					printf(
						__( 'There is a new version of %1$s available. <a href="%2$s" %3$s>View version %4$s details</a>.', 'cocart-pro' ),
						$plugin_name,
						esc_url( $details_url ),
						sprintf(
							'class="thickbox open-plugin-details-modal" aria-label="%s"',
							/* translators: 1: plugin name, 2: version number */
							esc_attr( sprintf( __( 'View %1$s version %2$s details', 'cocart-pro' ), $plugin_name, $response->new_version ) )
						),
					...
					/* translators: 1: plugin name, 2: details URL, 3: additional link attributes, 4: version number */
					printf(
						__( 'There is a new version of %1$s available. <a href="%2$s" %3$s>View version %4$s details</a>. <em>Automatic update is unavailable for this plugin.</em>', 'cocart-pro' ),
						$plugin_name,
						esc_url( $details_url ),
						sprintf(
							'class="thickbox open-plugin-details-modal" aria-label="%s"',
					...
					if ( $compatible_php ) {
						...
						printf(
							__( 'There is a new version of %1$s available. <a href="%2$s" %3$s>View version %4$s details</a> or <a href="%5$s" %6$s>update now</a>.', 'cocart-pro' ),
							$plugin_name,
							esc_url( $details_url ),
							sprintf(
								'class="thickbox open-plugin-details-modal" aria-label="%s"',
							...
							sprintf(
								...
								esc_attr( sprintf( __( 'Update %s now', 'cocart-pro' ), $plugin_name ) )
							)
						);
					} else {
						/* translators: 1: plugin name, 2: details URL, 3: additional link attributes, 4: version number 5: Update PHP page URL */
						printf(
							__( 'There is a new version of %1$s available, but it doesn&#8217;t work with your version of PHP. <a href="%2$s" %3$s>View version %4$s details</a> or <a href="%5$s">learn more about updating PHP</a>.', 'cocart-pro' ),
							$plugin_name,
							esc_url( $details_url ),
							sprintf(
								'class="thickbox open-plugin-details-modal" aria-label="%s"',
			...
			<script type="text/javascript">
				(function( $ ) {
					...
					if ( next_row.hasClass( 'plugin-update-tr' ) && !next_row.hasClass( 'cocart-pro-custom' ) ) {
						var original = next_row.clone();
						original.add;
						next_row.html( next_row.next().html() ).addClass( 'cocart-pro-custom-visible' );
						next_row.next().remove();
						next_row.after( original );
						original.addClass( 'cocart-pro-original-update-row' ).css('display', 'none');
					}
				})( jQuery );
			</script>
			<?php
