    ...
    class acf_field_post_object extends acf_field
    {
		...
		*/
        ...
        function format_value($value, $post_id, $field)
        {
            ...
            $path = parse_url(get_permalink($value))['path'];
            $thumbnail = get_the_post_thumbnail_url($value);
            $excerpt = get_the_excerpt($value);
            ...
            if ($field['return_format'] == 'object') {
                ...
                $posts = $this->get_posts($value, $field);
                foreach ($posts as $post) {
                    ...
                    if ($post->post_type == 'product') {
                        ...
                        foreach ($product_image_ids as $product_image_id) {
                            array_push($post->images, [
                                'id' => $product_image_id,
                                'src' => wp_get_attachment_url($product_image_id),
                                'name' => get_the_title($product_image_id),
                    ...
                    foreach (get_post_taxonomies($post->ID) as &$taxonomy) {
                        $taxonomies[$taxonomy] = get_the_terms($post->ID, $taxonomy);
                    }
                    $post->taxonomies = $taxonomies;
                    $post->title = $post->post_title;
                    $post->content = ['rendered' => $post->post_content];
                    $post->route = ['path' => parse_url(get_permalink($post->ID))['path']];
                    $post->image = ['sizes' => [
                        'thumbnail' => get_the_post_thumbnail_url($post->ID, 'thumbnail'),
                        'medium' => get_the_post_thumbnail_url($post->ID, 'medium'),
                        'large' => get_the_post_thumbnail_url($post->ID, 'large'),
                        'full_size' => get_the_post_thumbnail_url($post->ID, 'full_size'),
                    ]];
