...
class CoCart_Pro_Coupon_Controller extends CoCart_API_Controller {
	...
	public function get_applied_coupons( $data = array() ) {
		...
		/**
		 * This forces the formatting of the returned applied coupons 
		 * to return as an array even if a coupon was deleted at some point.
		 */
		if ( ! empty( $applied_coupons ) ) {
			$coupons = array();

			foreach ( $applied_coupons as $i => $coupon ) {
				$coupons[] = wc_format_coupon_code( wp_unslash( $coupon ) );
			}
