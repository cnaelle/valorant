            ...
            <section id="header" class="h-40 w-full bg-black/20 backdrop-blur">
                <div id="menu" class="container mx-auto flex h-full items-center justify-between px-4">
                    ...
                    <div id="nav" class="flex gap-x-4">
                        <button v-for="category, index in agentCategories" class="text-white valorant"
                            @click="afficher(category)">
                            <img :src="icons[index]" class="w-10 mx-auto pb-3">
                            {{ category }}
                        </button>
                        <input class="bg-slate-900/0 text-white border-1 h-9 w-40 pl-2 my-auto rounded-md" id="searchBar"
                            type="text" v-model="input" placeholder="Search an agent..." />
